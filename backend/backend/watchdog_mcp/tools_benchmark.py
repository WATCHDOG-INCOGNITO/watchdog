...
def register(mcp):
    ...
    @mcp.tool()
    async def benchmark_scan(run_id: str) -> str:
        """스캔의 전체 벤치마크를 출력한다.
        ...
        result = await sync_to_async(_query, thread_sensitive=False)()
        return json.dumps(result)

    @mcp.tool()
    async def benchmark_finding_detail(run_id: str) -> str:
    ...
    @mcp.tool()
    async def benchmark_compare(run_ids: str) -> str:
        """여러 스캔 결과를 비교한다.
        ...
        comparisons = await sync_to_async(_query, thread_sensitive=False)()
        return json.dumps({"scans": comparisons})

    @mcp.tool()
    async def benchmark_token_breakdown(run_id: str) -> str:
        ...
        def _query():
            ...
            return {
                ...
                "efficiency": {
                    ...
                    "tokens_per_finding": round(total_tokens / max(findings_count, 1), 1),
                    "cost_per_finding_usd": round(float(scan.llm_cost_usd) / max(findings_count, 1), 4),
                    "attempts_per_finding": round(verify_count / max(findings_count, 1), 1),
                },
            }
